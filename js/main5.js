...
function startLoadingSounds() {
	...
	levelsRequest = new XMLHttpRequest();
	levelsRequest.open("GET", "sounds/Avicii - Levels.mp3", true);
	levelsRequest.responseType = "arraybuffer";
	levelsRequest.onload = function() {
	  audioContext.decodeAudioData( levelsRequest.response, function(buffer) { 
	    	levelsBuffer = buffer; 
	    	buffers[6]= levelsBuffer;
		} );
	}
	levelsRequest.send();
...
else if(key== 102)
{
	...
	b.audioNode.frequency.value*=1.05;
}
else if(key == 103){//G
	b.audioNode.frequency.value/=1.05;
}
